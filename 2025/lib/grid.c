...
/**
 * Creates a new grid from a string view. The grid maintains no ownership over its string data.
 * @param data The input data.
 * @return A grid, or nullptr if the grid could not be parsed.
 */
struct Grid* grid_create(const struct StringView data) {
    struct Grid* grid = malloc(sizeof(struct Grid));

    grid->data = data;
    ptrdiff_t cursor = string_view_find_char(&grid->data, '\n', 0);
    ...
    do {
        size_y += 1;
        const ptrdiff_t old_cursor = cursor;
        cursor = string_view_find_char(&grid->data, '\n', cursor + 1);

        if (cursor != -1 && cursor - old_cursor != grid->size_x) {
...
char grid_get_square(const struct Grid* grid, const ptrdiff_t x, const ptrdiff_t y) {
    ...
    const ptrdiff_t idx = (y * grid->size_x) + x;
    return grid->data.data[idx];
}
...
struct GridAdjacentSquares grid_get_adjacent_squares(const struct Grid* grid, const ptrdiff_t x,
    ...
    for (ptrdiff_t x_search = x - 1; x_search <= x + 1; x_search++) {
        for (ptrdiff_t y_search = y - 1; y_search <= y + 1; y_search++) {
            ...
            if (square != 0) {
                assert(result.squares_len < 8);
                result.squares[result.squares_len] = square;
                result.squares_len += 1;
            }
        }
    }
...
void grid_free(struct Grid* grid) { free(grid); }